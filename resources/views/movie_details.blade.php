    ...
    <body>
        <div class="w-full h-screen flex flex-col relative">
            ...
            <div class="w-full h-full z-10 flex flex-col justify-center px-20">
                ...
                <span class="font-inter italic text-2xl mt-4 text-white max-w-3xl live-clamp-5">{{$tagline}}</span>
                ...
                <div class="flex flex-row mt-4 items-center">
                    ...
                    {{-- year --}}
                    <span class="font-inteer text-xl text-white bg-transparent rounded-md border border-white p-2 mr-4">{{$year}}</span>

                        {{-- duration --}}
                    @if($duration)
                    <span class="font-inteer text-xl text-white bg-transparent rounded-md border border-white p-2 mr-4">{{$duration}}</span>
                    @endif
                </div>

                    {{-- play trailer --}}
                    @if ($trailerID)
                <button class="w-fit bg-blue-500 text-white pl-4 pr-6 py-3 mt-5 font-inter text-xl flex flex-row rounded-lg items-center hover:drop-shadow-lg duration-200" onclick="showTrailer(true)">
            ...
            <div id="trailerWrapper" class="absolute z-10 w-full h-screen p-20 bg-black flex flex-col">
                <button class="ml-auto group mb-4" onclick="showTrailer(false)">
                  ...
                  <iframe id="youtubeVideo" class="w-full h-full" src="https://www.youtube.com/embed/{{$trailerID}}?enablejsapi=1" title="{{$movieData->title}}" frameborder="0" allow="accelerometer; autoplay; clipbord-write; ancrypted-media; gyroscape; picture-in-picture; web-share" allowfullscreen></iframe>
            </div>
        </div>
